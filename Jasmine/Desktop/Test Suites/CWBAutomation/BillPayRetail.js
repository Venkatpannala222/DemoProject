describe("BillPayRetail", function() {
	...
	it("SheduledPayment_Weekly", async function() {
	  ...
	  await SelectRecurringPaymentsTab();
	  await RecurringPaymentsTabValidation();
	  await EnterAmount_ScheduleBillPay(BillPay.schedulePay.amountValue2);
	  // await SelectPayFromAcc(Pay);
	  await SelectFrequency_ScheduleBillPay("Weekly");
	  await SelectDateRange_ScheduleBillPay();
	  await SelectPayee_ManagePayeeList(BillPay.schedulePay.PayeeName3);
	  await ConfirmScheduleBillPay();
	  await ClickOnConfirm();
	  await VerifyScheduleBillPaySuccessMsg();
	  await verifyAccountsLandingScreen();
